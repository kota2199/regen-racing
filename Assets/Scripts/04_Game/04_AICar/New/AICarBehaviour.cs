...
public class AICarBehaviour : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        AICarControll = gameObject.GetComponentInParent(typeof(AICarControll)) as AICarControll;
        AllWheelColliders = GetComponentsInChildren<WheelCollider>();
        NormalTorque = AICarControll.m_FullTorqueOverAllWheels;
        NormalSteering = AICarControll.m_MaximumSteerAngle;
        NormalTopspeed = AICarControll.m_Topspeed;
    }
    ...
    private void Update()
    {
        ...
        if (StartReverse == 1)//starting the reverse:
        {
            ...
            AICarControll.m_FullTorqueOverAllWheels = (AICarControll.m_FullTorqueOverAllWheels * -1);
            AICarControll.m_MaximumSteerAngle = 0;//the car won't turn
    ...
    {      
        ...
        AICarControll.m_FullTorqueOverAllWheels = AICarControll.m_FullTorqueOverAllWheels + NormalTorque + NormalTorque;
        //after one second, the car will be able to turn again
        yield return new WaitForSeconds(1);
        AICarControll.m_MaximumSteerAngle = NormalSteering;
    ...
    void OnTriggerEnter(Collider col)
    {
        ...
        {
            AICarControll.m_Topspeed = 15;//brakes are actually reducing the topspeed of the car
        }
    }
    ...
    void OnTriggerExit(Collider col)
    {
        if (col.tag == "Player" || col.tag == "AICarCollider")
        {
            AICarControll.m_Topspeed = NormalTopspeed;//bring back the normal topspeed
        }
    }
}
