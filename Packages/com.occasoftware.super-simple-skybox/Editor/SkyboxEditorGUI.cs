...
namespace OccaSoftware.SuperSimpleSkybox.Editor
{
	public class SkyboxEditorGUI : ShaderGUI
	{
		...
		public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
		{
			...
			MaterialProperty horizonSaturationFalloff = FindProperty(
				"_HorizonSaturationFalloff",
				properties
			);
			MaterialProperty horizonSaturationAmount = FindProperty(
				"_HorizonSaturationAmount",
				properties
			);
			...
			MaterialProperty sunSkyLightingEnabled = FindProperty(
				"_SunSkyLightingEnabled",
				properties
			);

			// Sunset
			MaterialProperty sunsetHorizontalFalloff = FindProperty(
				"_SunsetHorizontalFalloff",
				properties
			);
			MaterialProperty sunsetVerticalFalloff = FindProperty(
				"_SunsetVerticalFalloff",
				properties
			);
			...
			MaterialProperty starDaytimeBrightness = FindProperty(
				"_StarDaytimeBrightness",
				properties
			);
			MaterialProperty starSaturation = FindProperty("_StarSaturation", properties);
			MaterialProperty proceduralStarsEnabled = FindProperty(
				"_ProceduralStarsEnabled",
				properties
			);
			...
			void DrawCommonProperties()
			{
				...
				constantColorMode.floatValue = (float)
					(ColorMode)
						EditorGUILayout.EnumPopup(
							"Color Mode",
							(ColorMode)constantColorMode.floatValue
						);
				...
				if (cloudsEnabled.floatValue == 1.0f)
				{
					...
					if (cloudTexture.textureValue != null)
					{
						Vector2Int s = EditorGUILayout.Vector2IntField(
							"Scale",
							new Vector2Int(
								(int)cloudScale.vectorValue.x,
								(int)cloudScale.vectorValue.y
							)
						);
						cloudScale.vectorValue = new Vector4(s.x, s.y, 0, 0);

						cloudWindSpeed.vectorValue = EditorGUILayout.Vector2Field(
							"Speed",
							cloudWindSpeed.vectorValue
						);
						...
						EditorGUI.indentLevel++;
						string title =
							currentColorMode == ColorMode.Constant ? "Cloud Color" : "Day";
